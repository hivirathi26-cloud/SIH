...
export const JharkhandHeatmap = ({ selectedDistrict, onSelectDistrict }) => {
    const [metric, setMetric] = useState("total");
    return (<div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100">
        ...
        <div className="flex items-center space-x-1.5 bg-slate-100 p-1 rounded-xl text-xs">
          <button onClick={() => setMetric("total")} className={`px-3 py-1 rounded-lg font-medium transition ${metric === "total" ? "bg-white text-slate-900 shadow-xs font-bold" : "text-slate-600"}`}>
            Submissions
          </button>
          <button onClick={() => setMetric("resolved")} className={`px-3 py-1 rounded-lg font-medium transition ${metric === "resolved" ? "bg-white text-slate-900 shadow-xs font-bold" : "text-slate-600"}`}>
            Resolved
          </button>
          <button onClick={() => setMetric("vulnerability")} className={`px-3 py-1 rounded-lg font-medium transition ${metric === "vulnerability" ? "bg-white text-slate-900 shadow-xs font-bold" : "text-slate-600"}`}>
            Vulnerability
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2.5 max-h-96 overflow-y-auto pr-1">
        {JHARKHAND_DISTRICTS.map((d) => {
            ...
            return (<button key={d.id} onClick={() => onSelectDistrict(d.name)} className={`p-3 rounded-xl border text-left transition transform hover:scale-102 flex flex-col justify-between ${isSelected
