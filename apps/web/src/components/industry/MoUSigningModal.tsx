...
import { FileSignature, ShieldCheck, Download, CheckCircle2, X } from "lucide-react";
import confetti from "canvas-confetti";

export const MoUSigningModal: React.FC<{
  proposal: Proposal | null;
  onClose: () => void;
}> = ({ proposal, onClose }) => {
  const { createAgreement, currentUser } = useApp();
  const [agreementType, setAgreementType] = useState<any>("csr_grant");
  ...
  const handleSignAgreement = (e: React.FormEvent) => {
    ...
    createAgreement({
      ...
      industryType: (currentUser.role as any) === "csr" ? "csr" : "startup",
      ...
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 max-w-2xl w-full p-6 animate-in fade-in zoom-in-95 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input
                ...
                className="w-full p-2 border border-slate-300 rounded-lg bg-white text-xs font-mono font-bold text-emerald-700"
              />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div
            ...
            className={`h-28 border-2 border-dashed rounded-xl flex flex-col items-center justify-center cursor-pointer transition ${
              signatureDrawn
