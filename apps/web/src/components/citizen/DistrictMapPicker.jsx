...
export const DistrictMapPicker = ({ selectedDistrict, onSelectDistrict }) => {
    return (<div className="space-y-3">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 max-h-48 overflow-y-auto p-1 border border-slate-200 rounded-xl bg-slate-50/50">
        {JHARKHAND_DISTRICTS.map((d) => {
            ...
            return (<button key={d.id} type="button" onClick={() => onSelectDistrict(d.name, d.latitude, d.longitude)} className={`flex items-center justify-between p-2 rounded-lg text-xs font-medium border text-left transition ${isSelected
