...
import { Rocket, Sparkles, ExternalLink, Award } from "lucide-react";

export const StartupIncubationBanner: React.FC<{
  proposalTitle: string;
  universityName: string;
}> = ({ proposalTitle, universityName }) => {
  return (
    <div className="bg-gradient-to-r from-purple-900 via-indigo-900 to-slate-900 text-white p-5 rounded-2xl shadow-lg border border-purple-500/30 flex flex-col md:flex-row items-center justify-between gap-4">
      ...
      <button
        onClick={() => alert("Redirecting to Jharkhand State Startup Portal (startup.jharkhand.gov.in) with pre-filled innovation dossier!")}
        className="shrink-0 flex items-center space-x-2 px-5 py-2.5 bg-gradient-to-r from-purple-500 to-indigo-500 hover:from-purple-600 hover:to-indigo-600 text-white rounded-xl text-xs font-bold shadow-md shadow-purple-500/30 transition hover:scale-105"
      >
