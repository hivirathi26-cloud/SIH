...
import { Plus, Clock, User, CheckCircle2, AlertCircle, Sparkles } from "lucide-react";

export const KanbanBoard: React.FC<{ teamId: string }> = ({ teamId }) => {
  ...
  const columns: { key: KanbanTask["status"]; label: string; bg: string; border: string }[] = [
  ...
  return (
    <div className="space-y-4">
      ...
      {isAdding && (
        <form onSubmit={handleQuickAdd} className="bg-white p-3 rounded-xl border border-emerald-300 shadow-sm flex items-center space-x-2 animate-in fade-in">
          <input
            type="text"
            value={newTaskTitle}
            onChange={(e) => setNewTaskTitle(e.target.value)}
            placeholder="e.g. Calibrate optical sensor in Angara field trial station..."
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {columns.map((col) => {
          ...
          return (
            <div
              key={col.key}
              className={`${col.bg} border ${col.border} rounded-2xl p-3 flex flex-col min-h-[380px]`}
            >
              ...
              <div className="flex-1 space-y-2.5 overflow-y-auto">
                {tasksInCol.length === 0 ? (
                  ...
                  tasksInCol.map((task) => (
                    <div
                      key={task.id}
                      className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition space-y-2"
                    >
                      ...
                      <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-[11px]">
                        ...
                        <select
                          ...
                          className="text-[10px] bg-slate-50 border border-slate-200 rounded px-1.5 py-0.5 text-slate-700 font-medium"
                        >
