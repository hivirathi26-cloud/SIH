...
import { Award, Trophy, Medal, ShieldCheck, Heart, Sparkles, MapPin } from "lucide-react";
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm text-center space-y-3 md:order-1 order-2">
          ...
          <div className="flex flex-wrap justify-center gap-1">
            {MOCK_LEADERBOARD[1].badges.map((b, i) => (
        ...
        <div className="bg-gradient-to-b from-amber-50 to-white p-8 rounded-3xl border-2 border-amber-400 shadow-lg text-center space-y-3 md:order-2 order-1 transform md:-translate-y-4">
          ...
          <div className="flex flex-wrap justify-center gap-1">
            {MOCK_LEADERBOARD[0].badges.map((b, i) => (
              <span key={i} className="text-[10px] bg-amber-100 text-amber-900 px-2.5 py-0.5 rounded-full font-bold">
                {b}
              </span>
            ))}
          </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm text-center space-y-3 md:order-3 order-3">
          ...
          <div className="flex flex-wrap justify-center gap-1">
            {MOCK_LEADERBOARD[2].badges.map((b, i) => (
              <span key={i} className="text-[10px] bg-emerald-50 text-emerald-800 px-2 py-0.5 rounded-full font-medium">
                {b}
              </span>
            ))}
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="divide-y divide-slate-100">
          {MOCK_LEADERBOARD.map((u) => (
            <div key={u.id} className="p-4 flex items-center justify-between text-xs hover:bg-slate-50 transition">
