...
export const HeiTeamsPage = () => {
    ...
    return (<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-in fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <Link to="/hei/proposals/new" className="flex items-center space-x-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold shadow-sm transition">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {teams.map((team) => (<div key={team.id} className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {team.members.map((m) => (<div key={m.id} className="bg-slate-50 p-2 rounded-xl border border-slate-200 flex items-center space-x-2 text-xs">
                    <img src={m.avatarUrl || "https://images.unsplash.com/photo-1539571696357-5a69c17a67c6?w=150"} alt="" className="w-7 h-7 rounded-full object-cover border border-slate-300 shrink-0"/>
