...
import { MapPin, Heart, Star, BrainCircuit } from "lucide-react";
export const MyProblemsPage = () => {
    ...
    const [selectedProbForRating, setSelectedProbForRating] = useState(null);
    ...
    return (<div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6 animate-in fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-2">
          <div className="bg-slate-100 p-1 rounded-xl flex items-center text-xs">
            <button onClick={() => setFilterView("all")} className={`px-3 py-1.5 rounded-lg font-medium transition ${filterView === "all" ? "bg-white text-slate-900 shadow-xs font-bold" : "text-slate-600"}`}>
              All State Challenges ({problems.length})
            </button>
            <button onClick={() => setFilterView("my")} className={`px-3 py-1.5 rounded-lg font-medium transition ${filterView === "my" ? "bg-white text-slate-900 shadow-xs font-bold" : "text-slate-600"}`}>
              My Submissions
            </button>
          </div>

          <Link to="/submit" className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold transition shadow-sm">
            + New Challenge
          </Link>
        </div>
      </div>
      ...
      <div className="space-y-6">
        {displayedProblems.map((prob) => {
            ...
            return (<div key={prob.id} className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm hover:shadow-md transition space-y-5">
              {/* Top Meta Info */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100">
                ...
                <div className="flex items-center space-x-2">
                  <button onClick={() => setSelectedProbForXAI(prob)} className="flex items-center space-x-1 text-xs font-bold text-indigo-700 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 px-3 py-1.5 rounded-xl transition">
                    <BrainCircuit className="w-3.5 h-3.5 text-indigo-600"/>
                    <span>Inspect Explainable AI &rarr;</span>
                  </button>

                  {(prob.status === "deployed" || prob.status === "closed") && (<button onClick={() => setSelectedProbForRating(prob)} className="flex items-center space-x-1 text-xs font-bold text-amber-800 bg-amber-50 hover:bg-amber-100 border border-amber-300 px-3 py-1.5 rounded-xl transition">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-3">
                  ...
                  {prob.feedbackComment && (<div className="bg-teal-50 border border-teal-200 p-3 rounded-xl text-xs space-y-1">
                ...
                <div className="space-y-3">
                  {prob.media && prob.media[0] && (<div className="h-32 rounded-xl overflow-hidden border border-slate-200">
                  ...
                  <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 space-y-1 text-xs">
                    ...
                    <div className="flex justify-between items-center pt-1 border-t border-slate-200">
                      <button onClick={() => upvoteProblem(prob.id)} className="flex items-center space-x-1 text-slate-700 hover:text-rose-600 text-xs font-semibold">
              ...
              <div className="pt-4 border-t border-slate-100 space-y-2">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-5 lg:grid-cols-10 gap-1 text-center">
                  {stepsList.map((stepName, sIdx) => {
                    ...
                    return (<div key={stepName} className={`p-2 rounded-xl border text-[10px] font-medium transition flex flex-col items-center justify-between ${isCurrent
                        ...
                        <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] mb-1 font-bold ${isCurrent ? "bg-white text-emerald-700" : isCompleted ? "bg-emerald-600 text-white" : "bg-slate-200 text-slate-600"}`}>
                          {stepNum}
                        </span>
      ...
      <CitizenRatingModal problem={selectedProbForRating} onClose={() => setSelectedProbForRating(null)}/>
    </div>);
};
