...
import { Sparkles, ArrowRight, PlusCircle, Users, GraduationCap, Briefcase, Search, CheckCircle2, MapPin, Heart, ChevronRight } from "lucide-react";
    ...
    return (<div className="space-y-16 pb-12">
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-slate-900 via-slate-900 to-slate-950 text-white pt-16 pb-24 px-4 sm:px-6 lg:px-8 rounded-b-3xl shadow-xl">
        ...
        <div className="relative max-w-5xl mx-auto text-center space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 pt-4">
            <Link to="/submit" className="flex items-center space-x-2 px-6 py-3.5 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white rounded-xl text-sm font-bold shadow-lg shadow-emerald-500/30 transition hover:scale-105">
              <PlusCircle className="w-5 h-5"/>
              <span>Submit a Societal Challenge</span>
            </Link>

            <Link to="/my-problems" className="flex items-center space-x-2 px-6 py-3.5 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 rounded-xl text-sm font-semibold transition">
              <span>Track Challenge Status</span>
              <ArrowRight className="w-4 h-4"/>
            </Link>

            <Link to="/industry/marketplace" className="flex items-center space-x-2 px-6 py-3.5 bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 border border-amber-500/30 rounded-xl text-sm font-semibold transition">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="relative">
              ...
              <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search challenges or districts..." className="text-xs pl-9 pr-3 py-2 border border-slate-300 rounded-xl bg-white focus:ring-2 focus:ring-emerald-500 focus:outline-none w-56 sm:w-64"/>
            </div>

            <select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)} className="text-xs px-3 py-2 border border-slate-300 rounded-xl bg-white focus:ring-2 focus:ring-emerald-500 focus:outline-none">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredProblems.map((prob) => (<div key={prob.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition overflow-hidden flex flex-col justify-between">
              <div>
                {/* Media Image Banner */}
                {prob.media && prob.media[0] && (<div className="h-44 w-full relative overflow-hidden bg-slate-100">
                    <img src={prob.media[0].storageUrl} alt={prob.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform"/>
                ...
                <div className="p-5 space-y-3">
                  ...
                  {prob.assignedUniversityName && (<div className="bg-slate-50 p-2 rounded-lg border border-slate-200 text-[11px] flex items-center space-x-2">
              ...
              <div className="p-5 pt-0 border-t border-slate-100 mt-2 flex items-center justify-between">
                <button type="button" onClick={() => upvoteProblem(prob.id)} className="flex items-center space-x-1.5 text-xs text-slate-600 hover:text-emerald-700 transition" title="I face this issue too">
                  <Heart className="w-4 h-4 text-rose-500"/>
                  <span className="font-bold">{prob.citizenSupportCount} Citizen Supports</span>
                </button>

                <Link to="/my-problems" className="text-xs font-bold text-emerald-600 hover:text-emerald-700 flex items-center space-x-1">
