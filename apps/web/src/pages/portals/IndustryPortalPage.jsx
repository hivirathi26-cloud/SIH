...
import { Briefcase, Building, FileSignature, Clock, CheckCircle2, Download, ShieldCheck, FileText, Info } from "lucide-react";
export const IndustryPortalPage = () => {
    ...
    const isProposalFunded = (pr) => {
        return pr.status === "funded" || agreements.some((a) => a.proposalId === pr.id || a.proposalTitle.toLowerCase() === pr.title.toLowerCase());
    };
    ...
    return (<PortalLayout portalTitle="Industry & CSR Co-Creation Marketplace" portalSubtitle="उद्योग एवं निगमित सामाजिक उत्तरदायित्व (CSR) पटल — Tata Steel CSR" navItems={navItems} activeTab={activeTab} setActiveTab={setActiveTab}>
      {/* 1. Open Innovation Marketplace */}
      {activeTab === "marketplace" && (<div className="space-y-4">
          ...
          <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-xs space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <div className="flex flex-wrap items-center gap-2 text-xs">
                ...
                <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="p-1.5 border border-slate-300 rounded bg-slate-50 text-xs focus:border-[#0f2942]">
            ...
            <div className="flex items-center space-x-2 pt-2 border-t border-slate-100 text-xs">
              ...
              <button onClick={() => setStatusFilter("all")} className={`px-2.5 py-1 rounded text-[11px] font-semibold transition ${statusFilter === "all" ? "bg-[#0f2942] text-white" : "bg-slate-100 text-slate-700 hover:bg-slate-200"}`}>
                All ({proposals.length})
              </button>
              <button onClick={() => setStatusFilter("open")} className={`px-2.5 py-1 rounded text-[11px] font-semibold transition ${statusFilter === "open" ? "bg-[#0f2942] text-white" : "bg-slate-100 text-slate-700 hover:bg-slate-200"}`}>
                Open for Funding ({openCount})
              </button>
              <button onClick={() => setStatusFilter("funded")} className={`px-2.5 py-1 rounded text-[11px] font-semibold transition ${statusFilter === "funded" ? "bg-[#0f2942] text-white" : "bg-slate-100 text-slate-700 hover:bg-slate-200"}`}>
          ...
          <div className="space-y-4">
            {filteredProposals.map((pr) => {
                ...
                return (<div key={pr.id} className={`bg-white p-5 rounded-lg border shadow-xs space-y-3 transition ${funded ? "border-emerald-300 bg-emerald-50/10" : "border-slate-200"}`}>
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2 border-b border-slate-100 text-xs">
                    <div className="flex items-center space-x-2">
                      ...
                      {funded ? (<span className="bg-emerald-100 text-emerald-900 border border-emerald-300 text-[10px] font-bold px-2 py-0.5 rounded flex items-center space-x-1">
                          <CheckCircle2 className="w-3 h-3 text-emerald-700"/>
                          <span>MOU EXECUTED & ACTIVE</span>
                        </span>) : (<span className="bg-amber-50 text-amber-900 border border-amber-300 text-[10px] font-bold px-2 py-0.5 rounded">
                          OPEN FOR CO-FUNDING
                        </span>)}
                    </div>
                    ...
                    <div className="text-right">
                      ...
                      {funded && (<span className="text-[10px] text-slate-500 font-medium block">
                  ...
                  <div className="pt-2 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
                    ...
                    {funded ? (<div className="flex flex-wrap items-center gap-2">
                        ...
                        <button onClick={() => setActiveTab("agreements")} className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded text-xs font-semibold flex items-center space-x-1">
                          <FileText className="w-3.5 h-3.5"/>
                          <span>View Agreement</span>
                        </button>

                        <Link to={`/lifecycle/${pr.id}`} className="px-3.5 py-1.5 bg-[#0f2942] hover:bg-[#163b5f] text-white rounded text-xs font-semibold flex items-center space-x-1.5 shadow-xs">
                          <Clock className="w-3.5 h-3.5 text-amber-400"/>
                          <span>Track Live Project Lifecycle &rarr;</span>
                        </Link>
                      </div>) : (<button onClick={() => setSigningProposal(pr)} className="px-4 py-1.5 bg-[#0f2942] hover:bg-[#163b5f] text-white rounded font-semibold flex items-center space-x-1.5 shadow-xs">
      ...
      {activeTab === "agreements" && (<div className="space-y-4">
          ...
          <div className="space-y-4">
            {agreements.map((a) => (<div key={a.id} className="bg-white p-5 rounded-lg border border-emerald-300 shadow-xs space-y-3 text-xs">
                ...
                <div className="pt-2 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-2 text-[11px]">
                  ...
                  <div className="flex items-center space-x-2">
                    <Link to={`/lifecycle/${a.proposalId}`} className="px-3 py-1.5 bg-[#0f2942] hover:bg-[#163b5f] text-white rounded font-semibold flex items-center space-x-1">
                      <Clock className="w-3 h-3 text-amber-400"/>
                      <span>Track 5-Stage Project Milestones &rarr;</span>
                    </Link>

                    <button onClick={() => alert("Downloading official Jharkhand Innovation MoU (PDF with digital e-Sign timestamp)...")} className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded font-semibold flex items-center space-x-1">
      ...
      {/* 3. CSR Portfolio */}
      {activeTab === "portfolio" && (<div className="bg-white p-5 rounded-lg border border-slate-200 shadow-xs space-y-4 text-xs">
          <h3 className="font-heading font-bold text-sm text-slate-900">
