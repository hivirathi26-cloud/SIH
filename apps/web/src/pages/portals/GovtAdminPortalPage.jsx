...
export const GovtAdminPortalPage = () => {
    ...
    return (<PortalLayout portalTitle="State Apex Command & District Portal" portalSubtitle="राज्य शीर्ष नवाचार एवं जिला दंडाधिकारी पटल — Dr. Shailesh Kumar, IAS" navItems={navItems} activeTab={activeTab} setActiveTab={setActiveTab}>
      {/* 1. Apex Overview */}
      {activeTab === "apex_overview" && (<div className="space-y-6">
          {/* Header Banner */}
          <div className="bg-white p-5 rounded-lg border border-slate-200 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center space-x-2 shrink-0">
              <button onClick={handleExportPDF} className="px-3 py-1.5 bg-[#0f2942] hover:bg-[#163b5f] text-white text-xs font-semibold rounded flex items-center space-x-1">
                <Download className="w-3.5 h-3.5"/>
                <span>Export PDF</span>
              </button>
              <button onClick={handleExportExcel} className="px-3 py-1.5 border border-slate-300 hover:bg-slate-100 text-slate-700 text-xs font-semibold rounded flex items-center space-x-1">
          ...
          <JharkhandHeatmap selectedDistrict={selectedDistrict} onSelectDistrict={(d) => setSelectedDistrict(d)}/>
      ...
      {activeTab === "heatmap" && (<div className="space-y-4">
          <JharkhandHeatmap selectedDistrict={selectedDistrict} onSelectDistrict={(d) => setSelectedDistrict(d)}/>
        </div>)}

      {/* 3. Human AI Triage Validation Gate */}
      {activeTab === "ai_gate" && (<div className="space-y-4 text-xs">
          ...
          <div className="space-y-3">
            {pendingValidation.map((p) => {
                ...
                const matchPct = topMatch ? Math.round(topMatch.score > 1 ? topMatch.score : topMatch.score * 100) : 0;
                
                return (<div key={p.id} className="bg-white p-5 rounded-lg border border-slate-200 shadow-xs space-y-3">
      ...
      {activeTab === "dual_signoff" && (<div className="space-y-4 text-xs">
          ...
          <div className="space-y-4">
            {milestones.map((m) => {
                ...
                return (<div key={m.id} className={`bg-white p-5 rounded-lg border shadow-xs space-y-3 transition ${isApproved ? "border-emerald-300 bg-emerald-50/10" : "border-slate-200"}`}>
      ...
      {activeTab === "blockchain" && (<div className="space-y-4">
          <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-xs flex items-center justify-between">
            <div>
              <h3 className="font-heading font-bold text-sm text-slate-900">
            ...
            <button onClick={() => setBlockchainModalOpen(true)} className="px-3.5 py-1.5 bg-[#0f2942] text-white text-xs font-semibold rounded hover:bg-[#163b5f]">
      ...
      {/* 6. Reports & Export */}
      {activeTab === "reports" && (<div className="bg-white p-5 rounded-lg border border-slate-200 shadow-xs space-y-4 text-xs">
          <h3 className="font-heading font-bold text-sm text-slate-900">
          ...
          <div className="flex items-center space-x-3 pt-2">
            <button onClick={handleExportPDF} className="px-4 py-2 bg-[#0f2942] text-white font-semibold rounded flex items-center space-x-1.5">
              <Download className="w-3.5 h-3.5"/>
              <span>Download Comprehensive Annual State Innovation Report (PDF)</span>
            </button>
            <button onClick={handleExportExcel} className="px-4 py-2 border border-slate-300 text-slate-700 font-semibold rounded hover:bg-slate-50 flex items-center space-x-1.5">
              <FileSpreadsheet className="w-3.5 h-3.5"/>
