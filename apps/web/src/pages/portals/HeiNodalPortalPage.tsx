...
export const HeiNodalPortalPage: React.FC = () => {
  ...
  // Filter routed problems awaiting HEI acceptance
    ...
    >
          ...
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-heading font-bold text-sm text-slate-900">
                Challenges Routed by State AI Engine ({routedProblems.length})
              </h3>
                ...
                <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <button
                      ...
                      className="px-4 py-1.5 bg-[#0f2942] hover:bg-[#163b5f] text-white rounded font-semibold"
                    >
                      Accept Challenge & Assign Faculty &rarr;
                    </button>
                  </div>
                </div>
              </div>
      ...
      {activeTab === "accepted" && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-heading font-bold text-sm text-slate-900">
              Accepted Challenges in Active Pipeline ({acceptedProblems.length})
            </h3>
          </div>

          <div className="space-y-3">
            {acceptedProblems.map((p) => (
              <div key={p.id} className="bg-white p-4 rounded-lg border border-slate-200 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                <div>
                  <div className="flex items-center space-x-2 mb-1">
                    <span className="font-mono font-bold text-slate-600">{p.ticketNumber}</span>
                    <StatusPill status={p.status} />
                  </div>
                  <h4 className="font-heading font-bold text-xs text-slate-900">{p.title}</h4>
                  <p className="text-[11px] text-slate-500">
                    Faculty Assigned: <strong>{p.assignedFacultyName || "Prof. Ananya Sen"}</strong> • District: {p.district}
                  </p>
                </div>

                <div className="shrink-0 text-right">
                  <span className="text-[11px] text-slate-500 block">Current Stage:</span>
                  <span className="font-semibold text-emerald-800 uppercase font-mono">{p.status.replace("_", " ")}</span>
                </div>
              </div>
            ))}
          </div>
