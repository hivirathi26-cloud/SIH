...
import { Building, CheckCircle2, Users, BrainCircuit, FileText } from "lucide-react";
import confetti from "canvas-confetti";
export const HeiNodalPortalPage = () => {
    ...
    // "Routed Challenges" are those awaiting acceptance for this university (excluding direct line-dept healthcare/water issues)
    const routedProblems = problems.filter((p) => {
        const isPending = p.status === "routed" || p.status === "pending_nodal_review";
        if (!isPending)
            return false;
        
        // Exclude Healthcare and Water line-department problems
        ...
        return isAssignedToUs || isTopSuggested;
    });
    // "Accepted Challenges" are strictly those assigned to this institution
    const acceptedProblems = problems.filter((p) => p.assignedUniversityId === eco.id &&
    ...
    return (<PortalLayout portalTitle="University Nodal Desk (HEI Portal)" portalSubtitle={`विश्वविद्यालय एवं उच्च शिक्षण संस्थान नवाचार पटल — ${eco.name} (${eco.shortName})`} navItems={navItems} activeTab={activeTab} setActiveTab={setActiveTab}>
      {/* 1. Overview Tab */}
      ...
      {activeTab === "routed" && (<div className="space-y-4">
          ...
          {routedProblems.length === 0 ? (<div className="bg-white p-8 rounded-lg border border-slate-200 text-center space-y-2">
              ...
              <button onClick={() => setActiveTab("accepted")} className="mt-2 px-4 py-1.5 bg-[#0f2942] text-white text-xs font-semibold rounded">
                View Accepted Challenges ({acceptedProblems.length}) &rarr;
              </button>
            </div>) : (<div className="space-y-4">
              {routedProblems.map((p) => (<div key={p.id} className="bg-white p-5 rounded-lg border border-slate-200 shadow-xs space-y-3">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2 border-b border-slate-100 text-xs">
                    ...
                    <button onClick={() => setSelectedProbForXAI(p)} className="text-xs font-semibold text-blue-700 hover:text-blue-900 flex items-center space-x-1">
                  ...
                  <div className="pt-2 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
                    ...
                    <div className="flex items-center space-x-2">
                      <button onClick={() => handleDecline(p.id)} className="px-3 py-1.5 border border-slate-300 text-slate-700 hover:bg-slate-100 rounded font-semibold">
                        Decline & Re-Route
                      </button>
                      <button onClick={() => handleAccept(p.id)} className="px-4 py-1.5 bg-[#0f2942] hover:bg-[#163b5f] text-white rounded font-semibold flex items-center space-x-1">
      ...
      {activeTab === "accepted" && (<div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-heading font-bold text-sm text-slate-900">
          ...
          <div className="space-y-3">
            {acceptedProblems.map((p) => (<div key={p.id} className="bg-white p-5 rounded-lg border border-emerald-300 shadow-xs space-y-3 text-xs">
                ...
                <div className="pt-2 border-t border-slate-100 flex items-center justify-between">
                  ...
                  <Link to="/portal/faculty" className="px-3 py-1.5 bg-[#0f2942] hover:bg-[#163b5f] text-white rounded font-semibold flex items-center space-x-1">
      ...
      {activeTab === "faculty_roster" && (<div className="bg-white rounded-lg border border-slate-200 shadow-xs overflow-hidden">
      ...
      {activeTab === "proposals" && (<div className="space-y-4">
          ...
          <div className="space-y-3">
            {univProposals.map((pr) => (<div key={pr.id} className="bg-white p-4 rounded-lg border border-slate-200 shadow-xs space-y-2 text-xs">
