...
import { GraduationCap, Users, FileText, CheckCircle2, Send, Plus, ShieldCheck, Building, Clock, Info, CheckSquare, Download, X, Eye } from "lucide-react";
import confetti from "canvas-confetti";
export const FacultyPortalPage = () => {
    const { problems, teams, proposals, milestones, createProposal, approveMilestoneFaculty, studentDeliverables, reviewStudentDeliverable, assignStudentTask, currentUser } = useApp();
    ...
    const assignedProblems = problems.filter((p) => p.assignedUniversityId === eco.id ||
    ...
    const univTeams = teams.filter((t) => t.universityId === eco.id || t.facultyMentorId === eco.faculty.id || t.facultyMentorName === eco.faculty.fullName);
    ...
    const [lastSubmittedProposalId, setLastSubmittedProposalId] = useState(null);
    ...
    const univDeliverables = studentDeliverables.filter((d) => {
        ...
        const isOurProposal = proposals.some((pr) => pr.id === d.proposalId &&
    ...
    const pendingDeliverables = univDeliverables.filter((d) => d.status === "in_review_by_faculty");
    ...
    const handleSelectProblemForProposal = (p) => {
    ...
    const handleReviewDeliverable = (taskId, decision) => {
        ...
        reviewStudentDeliverable(taskId, decision, feedback, currentUser?.fullName || `${eco.faculty.fullName} (${eco.shortName})`);
        if (decision === "accept") {
    ...
    const filteredDeliverables = univDeliverables.filter((d) => {
        if (deliverableFilter === "pending")
            return d.status === "in_review_by_faculty";
        if (deliverableFilter === "approved")
            return d.status === "approved_by_faculty";
        if (deliverableFilter === "revisions")
            return d.status === "revision_requested";
        return true;
    });
    return (<PortalLayout portalTitle="Faculty Mentor Workspace" portalSubtitle={`प्राध्यापक परामर्शदाता एवं शोध पटल — ${eco.name} (${eco.shortName})`} navItems={navItems} activeTab={activeTab} setActiveTab={setActiveTab}>
      {/* 1. Assigned Challenges */}
      {activeTab === "challenges" && (<div className="space-y-4">
          ...
          <div className="space-y-4">
            {assignedProblems.map((p) => {
                const existingTeam = teams.find((t) => t.problemId === p.id || t.problemTitle.toLowerCase() === p.title.toLowerCase());
                const existingProposal = proposals.find((pr) => pr.problemId === p.id || pr.problemTitle.toLowerCase() === p.title.toLowerCase() || pr.title.toLowerCase().includes(p.title.toLowerCase().slice(0, 20)));
                return (<div key={p.id} className={`bg-white p-5 rounded-lg border shadow-xs space-y-3 transition ${existingProposal ? "border-emerald-300 bg-emerald-50/10" : "border-slate-200"}`}>
                  ...
                  <div className="pt-2 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      {/* Team Status */}
                      {existingTeam ? (<button onClick={() => setActiveTab("team_builder")} className="bg-purple-50 hover:bg-purple-100 text-purple-900 border border-purple-200 font-bold px-3 py-1.5 rounded text-xs flex items-center space-x-1.5 transition" title="Click to view student roster">
                          <Users className="w-3.5 h-3.5 text-purple-700"/>
                          <span>✓ Team Formed ({existingTeam.members.length} Innovators)</span>
                        </button>) : (<button onClick={() => setSelectedProbForTeam(p)} className="px-3.5 py-1.5 bg-[#0f2942] hover:bg-[#163b5f] text-white rounded font-semibold flex items-center space-x-1.5">
                          <Users className="w-3.5 h-3.5"/>
                          <span>Form Student Cohort</span>
                        </button>)}

                      {/* Proposal Status */}
                      {existingProposal ? (<div className="flex items-center space-x-2">
                          ...
                          <Link to={`/lifecycle/${existingProposal.id}`} className="px-3 py-1.5 bg-[#0f2942] hover:bg-[#163b5f] text-white rounded text-xs font-semibold flex items-center space-x-1 shadow-xs">
                            <Clock className="w-3.5 h-3.5 text-amber-400"/>
                            <span>Track Milestones & Lifecycle &rarr;</span>
                          </Link>
                        </div>) : (<button onClick={() => handleSelectProblemForProposal(p)} className="px-3.5 py-1.5 bg-emerald-700 hover:bg-emerald-800 text-white rounded font-semibold flex items-center space-x-1">
      ...
      {/* 2. Team Builder */}
      {activeTab === "team_builder" && (<div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-heading font-bold text-sm text-slate-900">
            ...
            <button onClick={() => setSelectedProbForTeam(assignedProblems[0])} className="px-3 py-1.5 bg-[#0f2942] text-white text-xs font-semibold rounded hover:bg-[#163b5f]">
              + Assemble New Team
            </button>
          </div>

          <div className="space-y-4">
            {teams.map((t) => (<div key={t.id} className="bg-white p-5 rounded-lg border border-slate-200 shadow-xs space-y-4">
                <div className="flex items-center justify-between pb-2 border-b border-slate-100 text-xs">
                  <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  {t.members.map((m) => (<div key={m.id} className="bg-slate-50 p-3 rounded border border-slate-200 flex items-center justify-between">
                      <div>
      ...
      {/* 3. Draft Proposal */}
      {activeTab === "proposal" && (<div className="bg-white rounded-lg border border-slate-200 shadow-xs overflow-hidden">
          ...
          {lastSubmittedProposalId ? (<div className="p-8 text-center space-y-4">
              ...
              <div className="max-w-xl mx-auto bg-slate-50 border border-slate-200 rounded p-4 text-xs text-left space-y-2">
                <div className="flex items-center justify-between pb-1 border-b border-slate-200">
                  ...
                  <span className={`font-bold px-2 py-0.5 rounded text-[11px] ${needsIndustrySupport
              ...
              <div>
                ...
                <select value={selectedProblemId} onChange={(e) => {
                  ...
                  {assignedProblems.map((p) => (<option key={p.id} value={p.id}>
              ...
              <div>
                ...
                <input type="text" required value={propTitle} onChange={(e) => setPropTitle(e.target.value)} className="w-full p-2.5 border border-slate-300 rounded focus:border-[#0f2942] focus:outline-none"/>
              </div>
              ...
              <div>
                ...
                <textarea required rows={3} value={propSummary} onChange={(e) => setPropSummary(e.target.value)} className="w-full p-2.5 border border-slate-300 rounded focus:border-[#0f2942] focus:outline-none"/>
              </div>
              ...
              <div>
                ...
                <textarea required rows={3} value={propApproach} onChange={(e) => setPropApproach(e.target.value)} className="w-full p-2.5 border border-slate-300 rounded focus:border-[#0f2942] focus:outline-none"/>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block font-semibold text-slate-800 mb-1">Estimated R&D Budget (INR ₹)</label>
                  <input type="number" required value={propBudget} onChange={(e) => setPropBudget(Number(e.target.value))} className="w-full p-2 border border-slate-300 rounded font-mono font-bold text-emerald-800"/>
                </div>

                <div>
                  <label className="block font-semibold text-slate-800 mb-1">Project Duration (Months)</label>
                  <input type="number" required value={propDuration} onChange={(e) => setPropDuration(Number(e.target.value))} className="w-full p-2 border border-slate-300 rounded font-mono"/>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {/* Path A */}
                  <div onClick={() => setNeedsIndustrySupport(true)} className={`p-3.5 rounded-lg border cursor-pointer transition ${needsIndustrySupport
                    ...
                    <div className="flex items-start justify-between">
                      ...
                      <input type="radio" checked={needsIndustrySupport} onChange={() => setNeedsIndustrySupport(true)} className="w-4 h-4 text-[#0f2942] mt-0.5"/>
                    </div>
                  </div>

                  {/* Path B */}
                  <div onClick={() => setNeedsIndustrySupport(false)} className={`p-3.5 rounded-lg border cursor-pointer transition ${!needsIndustrySupport
                    ...
                    <div className="flex items-start justify-between">
                      ...
                      <input type="radio" checked={!needsIndustrySupport} onChange={() => setNeedsIndustrySupport(false)} className="w-4 h-4 text-[#0f2942] mt-0.5"/>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="pt-2 border-t border-slate-200 flex justify-end">
                <button type="submit" className="px-5 py-2.5 bg-[#0f2942] hover:bg-[#163b5f] text-white rounded font-semibold flex items-center space-x-1.5">
      ...
      {activeTab === "student_deliverables" && (<div className="space-y-4">
          <div className="bg-blue-50/70 border border-blue-200 p-3.5 rounded-lg flex items-start space-x-3 text-xs">
            <Info className="w-4 h-4 text-[#0f2942] shrink-0 mt-0.5"/>
            <div className="text-slate-700 leading-relaxed">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-3.5 rounded-lg border border-slate-200">
            <div className="flex flex-wrap items-center gap-1.5 text-xs">
              ...
              <button onClick={() => setDeliverableFilter("all")} className={`px-2.5 py-1 rounded font-semibold ${deliverableFilter === "all" ? "bg-[#0f2942] text-white" : "bg-slate-100 text-slate-700 hover:bg-slate-200"}`}>
                All ({studentDeliverables.length})
              </button>
              <button onClick={() => setDeliverableFilter("pending")} className={`px-2.5 py-1 rounded font-semibold ${deliverableFilter === "pending"
                ? "bg-amber-600 text-white"
                : "bg-amber-50 text-amber-900 border border-amber-200 hover:bg-amber-100"}`}>
                Pending Review ({pendingDeliverables.length})
              </button>
              <button onClick={() => setDeliverableFilter("approved")} className={`px-2.5 py-1 rounded font-semibold ${deliverableFilter === "approved"
                ? "bg-emerald-700 text-white"
                : "bg-emerald-50 text-emerald-900 border border-emerald-200 hover:bg-emerald-100"}`}>
                E-Signed & Approved ({studentDeliverables.filter((d) => d.status === "approved_by_faculty").length})
              </button>
              <button onClick={() => setDeliverableFilter("revisions")} className={`px-2.5 py-1 rounded font-semibold ${deliverableFilter === "revisions"
            ...
            <button onClick={() => setShowAssignTask(!showAssignTask)} className="px-3 py-1.5 bg-[#0f2942] hover:bg-[#163b5f] text-white text-xs font-semibold rounded flex items-center space-x-1 shrink-0">
          ...
          {showAssignTask && (<div className="bg-slate-50 border border-slate-300 rounded-lg p-4 space-y-3 text-xs animate-in fade-in">
              <div className="flex items-center justify-between pb-2 border-b border-slate-200">
                <h4 className="font-bold text-slate-900">
                  Assign Milestone Deliverable to Student Cohort Member
                </h4>
                <button onClick={() => setShowAssignTask(false)} className="text-slate-400 hover:text-slate-600">
                  <X className="w-4 h-4"/>
                </button>
              </div>
              ...
              <form onSubmit={handleAssignTaskByFaculty} className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select value={assignStudentId} onChange={(e) => setAssignStudentId(e.target.value)} className="w-full p-2 border border-slate-300 rounded bg-white text-slate-800 font-semibold">
                ...
                <div className="sm:col-span-2">
                  ...
                  <input type="text" required placeholder="e.g. Conduct NABL Spectrophotometric Fluoride Re-Test & Attach Sheet" value={assignTitle} onChange={(e) => setAssignTitle(e.target.value)} className="w-full p-2 border border-slate-300 rounded bg-white"/>
                </div>

                <div className="sm:col-span-2">
                  <label className="block font-semibold text-slate-700 mb-1">Technical Instructions / Scope</label>
                  <textarea rows={2} placeholder="Specify testing parameters, expected tolerance, and required PDF format..." value={assignDesc} onChange={(e) => setAssignDesc(e.target.value)} className="w-full p-2 border border-slate-300 rounded bg-white"/>
                </div>

                <div className="sm:col-span-2 flex justify-end space-x-2 pt-2">
                  <button type="button" onClick={() => setShowAssignTask(false)} className="px-3 py-1.5 border border-slate-300 rounded font-semibold text-slate-600 hover:bg-white">
                    Cancel
                  </button>
                  <button type="submit" className="px-4 py-1.5 bg-[#0f2942] hover:bg-[#163b5f] text-white font-semibold rounded">
          ...
          {/* Deliverables List */}
          <div className="space-y-4">
            {filteredDeliverables.length === 0 ? (<div className="bg-white p-8 rounded-lg border border-slate-200 text-center text-slate-500 text-xs">
                No deliverables found matching the selected filter.
              </div>) : (filteredDeliverables.map((task) => {
                ...
                return (<div key={task.id} className={`bg-white p-5 rounded-lg border shadow-xs space-y-3 text-xs transition ${isApproved
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2 border-b border-slate-100">
                      ...
                      <div>
                        {isApproved && (<span className="bg-emerald-100 text-emerald-900 border border-emerald-300 font-bold px-2.5 py-1 rounded text-[11px] flex items-center space-x-1">
                            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-700"/>
                            <span>ACADEMIC E-SIGN APPLIED</span>
                          </span>)}
                        {isUnderReview && (<span className="bg-amber-100 text-amber-950 border border-amber-300 font-bold px-2.5 py-1 rounded text-[11px] animate-pulse">
                            ⏳ ACTION REQUIRED: PENDING FACULTY E-SIGN
                          </span>)}
                        {isRevision && (<span className="bg-rose-100 text-rose-900 border border-rose-300 font-bold px-2.5 py-1 rounded text-[11px]">
                            ⚠️ REVISIONS REQUESTED
                          </span>)}
                        {!isApproved && !isUnderReview && !isRevision && (<span className="bg-slate-100 text-slate-700 font-bold px-2.5 py-1 rounded text-[11px]">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-slate-50 p-3 rounded border border-slate-200 text-[11px]">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                          <div className={`h-full ${task.progressPercent >= 100 ? "bg-emerald-600" : "bg-blue-600"}`} style={{ width: `${task.progressPercent}%` }}></div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white p-3 rounded border border-slate-200 space-y-2 text-[11px]">
                      <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => setInspectingDoc({
                        title: task.title,
                      ...
                      {task.submissionNotes && (<p className="text-slate-600 italic bg-slate-50 p-2 rounded border border-slate-100">
                          <strong>Student Observation:</strong> "{task.submissionNotes}"
                        </p>)}
                    </div>

                    {/* Verification Actions or Existing Stamps */}
                    {isApproved ? (<div className="bg-emerald-50/80 border border-emerald-300 p-3 rounded text-[11px] text-emerald-950 space-y-1">
                      ...
                      </div>) : (<div className="bg-slate-50 p-3.5 rounded-lg border border-slate-200 space-y-3">
                        <div>
                          <label className="block font-semibold text-slate-800 mb-1">
                            Academic Verification Notes & Laboratory Feedback:
                          </label>
                          <textarea rows={2} placeholder="Enter your formal observations regarding test parameters, calibration data, and safety tolerances..." value={feedbackNotes[task.id] || ""} onChange={(e) => setFeedbackNotes((prev) => ({ ...prev, [task.id]: e.target.value }))} className="w-full p-2 border border-slate-300 rounded bg-white text-xs focus:border-[#0f2942] focus:outline-none"/>
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2 border-t border-slate-200">
                          ...
                          <div className="flex items-center space-x-2">
                            <button onClick={() => handleReviewDeliverable(task.id, "reject")} className="px-3 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-800 border border-rose-300 font-semibold rounded text-xs">
                              ✕ Request Revisions
                            </button>

                            <button onClick={() => handleReviewDeliverable(task.id, "accept")} className="px-4 py-1.5 bg-emerald-700 hover:bg-emerald-800 text-white font-semibold rounded text-xs flex items-center space-x-1.5 shadow-xs">
      ...
      {activeTab === "milestone_gate" && (<div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-heading font-bold text-sm text-slate-900">
          ...
          <div className="space-y-4">
            {milestones.map((m) => {
                ...
                return (<div key={m.id} className={`bg-white p-5 rounded-lg border shadow-xs space-y-3 text-xs transition ${isFacultyApproved ? "border-emerald-300 bg-emerald-50/10" : "border-slate-200"}`}>
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2 border-b border-slate-100">
                    <div>
                      <span className="font-mono text-slate-500 text-[11px] block">
                    ...
                    <div>
                      {isFacultyApproved ? (<span className="bg-emerald-100 text-emerald-900 border border-emerald-300 font-bold px-2.5 py-1 rounded text-[11px] flex items-center space-x-1">
                          <CheckCircle2 className="w-3.5 h-3.5 text-emerald-700"/>
                          <span>FACULTY STAMP APPLIED</span>
                        </span>) : (<span className="bg-amber-50 text-amber-900 border border-amber-300 font-bold px-2 py-0.5 rounded text-[10px]">
                  ...
                  <div className="pt-2 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    ...
                    {!isFacultyApproved ? (<button onClick={() => {
                      ...
                      </button>) : (<span className="text-emerald-700 font-semibold flex items-center space-x-1">
                        <CheckCircle2 className="w-4 h-4"/>
      ...
      {inspectingDoc && (<div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-2xl border border-slate-300 w-full max-w-2xl overflow-hidden animate-in fade-in">
            ...
            <div className="bg-[#0f2942] text-white p-4 flex items-center justify-between">
              ...
              <button onClick={() => setInspectingDoc(null)} className="text-slate-300 hover:text-white">
                <X className="w-5 h-5"/>
              </button>
            </div>
            ...
            <div className="p-6 space-y-4 text-xs max-h-[75vh] overflow-y-auto">
              ...
              <div className="p-3 bg-blue-50/60 rounded border border-blue-200 text-slate-700 flex items-center justify-between">
                ...
                <button onClick={() => alert(`Downloading ${inspectingDoc.pdfName} from Document Vault...`)} className="px-3 py-1.5 bg-[#0f2942] text-white font-semibold rounded text-xs flex items-center space-x-1">
            ...
            <div className="p-3 border-t border-slate-200 bg-slate-50 flex justify-end">
              <button onClick={() => setInspectingDoc(null)} className="px-4 py-1.5 bg-slate-200 hover:bg-slate-300 font-semibold rounded text-xs text-slate-700">
