...
export const LoginPage = () => {
    ...
    const handleRoleQuickLogin = (roleKey) => {
        loginAsRole(roleKey);
        navigate(getPortalPath(roleKey));
    };
    return (<div className="max-w-4xl mx-auto px-4 sm:px-6 py-10 space-y-8 text-xs">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
        ...
        <div className="md:col-span-6 bg-white p-6 rounded-lg border border-slate-200 shadow-xs space-y-4">
          ...
          <form onSubmit={handleFormSubmit} className="space-y-4">
            <div>
              ...
              <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} placeholder="e.g. nodal.innovation@bitmesra.ac.in" className="w-full p-2.5 border border-slate-300 rounded focus:border-[#0f2942] focus:outline-none"/>
            </div>
            ...
            <div>
              ...
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••••••" className="w-full p-2.5 border border-slate-300 rounded focus:border-[#0f2942] focus:outline-none"/>
            </div>

            <button type="submit" className="w-full py-2.5 bg-[#0f2942] hover:bg-[#163b5f] text-white font-semibold rounded shadow-xs">
          ...
          {[
            ...
            },
            {
        ...
        ].map((group) => (<div key={group.category} className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                if (!u)
                    return null;
                return (<button key={k} onClick={() => handleRoleQuickLogin(k)} className="w-full text-left p-2 bg-white hover:bg-blue-50/60 border border-slate-200 rounded transition flex items-center justify-between group shadow-2xs">
                      <div>
