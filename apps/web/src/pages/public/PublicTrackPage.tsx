...
export const PublicTrackPage: React.FC = () => {
  ...
  const [searchedProblem, setSearchedProblem] = useState<any>(problems[0]);

  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    const found = problems.find(
      (p) => p.ticketNumber.toLowerCase() === ticketInput.trim().toLowerCase()
    );
    if (found) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10 space-y-6 text-xs">
      ...
      <form onSubmit={handleSearch} className="bg-white p-4 rounded-lg border border-slate-200 shadow-xs flex gap-3">
        <input
          ...
          placeholder="e.g. JSICP-2026-0841"
          className="flex-1 p-2.5 border border-slate-300 rounded font-mono font-bold text-slate-800 focus:border-[#0f2942] focus:outline-none"
        />
        <button
          type="submit"
          className="px-5 py-2.5 bg-[#0f2942] hover:bg-[#163b5f] text-white font-semibold rounded flex items-center space-x-1.5 shrink-0"
        >
          ...
          <div className="space-y-2 pt-2 border-t border-slate-100">
            ...
            <div className="w-full bg-slate-100 h-2.5 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${getStepIndex(searchedProblem.status) * 10}%` }}
              ></div>
            </div>
