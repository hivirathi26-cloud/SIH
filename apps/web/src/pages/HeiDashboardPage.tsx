...
export const HeiDashboardPage: React.FC = () => {
  ...
  // Filter problems routed to or relevant to HEIs
  const routedProblems = problems.filter(
    (p) => p.status === "routed" || p.status === "pending_nodal_review" || p.assignedUniversityId === "univ-bit-mesra"
  );
  const acceptedProblems = problems.filter(
    (p) => p.status === "accepted_by_hei" || p.status === "team_formed" || p.status === "in_progress"
  );
  ...
  const handleDeclineProblem = (problemId: string) => {
            ...
            >
              ...
              <div className="pt-4 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2">
                <button
                  ...
                  className="flex items-center space-x-1 text-xs font-bold text-indigo-700 hover:text-indigo-900"
                >
                ...
                <div className="flex items-center space-x-2">
                  <button
                    onClick={() => handleDeclineProblem(prob.id)}
                    className="px-3 py-1.5 text-xs font-semibold text-rose-700 hover:bg-rose-50 border border-rose-200 rounded-xl transition"
                  >
                    Decline & Re-Route
                  </button>

                  <button
                    onClick={() => handleAcceptProblem(prob.id)}
                    className="px-4 py-1.5 text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-xl shadow-sm transition hover:scale-105"
                  >
                    Accept Challenge ✓
                  </button>
                </div>
          ...
          {acceptedProblems.map((prob) => (
            ...
            >
              ...
              <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                <button
                  onClick={() => setSelectedProbForTeam(prob)}
                  className="flex items-center space-x-1.5 px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-xl text-xs font-bold transition shadow-sm"
                >
                  <Users className="w-4 h-4" />
